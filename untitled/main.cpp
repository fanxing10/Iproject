...
int *astate_arr = new int[g->num_nodes]; // the state of nodes  *
...
int *visited = new int[g->num_nodes];  //一次游走中记录节点是否被访问过
...
{
    cout << "--------------------------------------------------------------------------------" << endl;
    cout << "Start Reverse Walk) algorithm" << endl;
//	time_t time_start = time(NULL);
    cout << "No.\tnegative_influence\tpositive_influence\ttime(s)" << endl;
    srand((unsigned)time(NULL));
    // int *bi_arr = new int[g->num_nodes]; //the array of current active nodes         *
    int node,ka,num,con=0,no=1;             //存储选出的种子个数
//	int *similarity = new int[g->num_nodes]; //存储节点和种子集合的相似性
//	delete [] similarity;
//	int *visited = new int[g->num_nodes];  //一次游走中记录节点是否被访问过
//    int *walk_arr = new int[g->num_nodes]; //存储游走经过节点的次数
    int *ai=new int[g->num_nodes];  //存储根据B的游走选出的
//	double total_inf = 0;  //影响力
...
//		similarity[i] = 0;
    ...
    for(int i=0;i<s;i++)
    {
        switch(model)
        {
            case 1: num=distributor1(s,i,con,anode);con=con+num;break;
            case 2: num=distributor2(s,i,con,anode);con=con+num;break;
            case 3: num=distributor3(s,i,con,anode);con=con+num;break;
            case 4: num=distributor4(s,i,con,anode);con=con+num;break;
            case 5: num=distributor5(s,i,con,anode);con=con+num;break;
        }
        //      cout<<"****************"<<endl;
        ...
        {
            Pair m_pair(j,walk_arr[j]);
            tmp_pqueue.push(m_pair);  //用来存储此节点被经过了几次
        }
        for(int j=0; j<num; j++)//选k个种子节点
        {
            Pair s_pair = tmp_pqueue.top();
            tmp_pqueue.pop();
            aseed_arr[j] = s_pair.key;  //把此节点加入种子数组
            aseed_num[j] = s_pair.value;   //存储此种子被经过了几次
        }
        ka=num;   //需要选的种子节点数目
        cout << no << "\t\t";
...
void diffusion(int ka,int *aai)
{
   ...
   */
    ...
    for(int i = 0; i < ka; i++)              //初始化种子节点
    {
        ...
        astate_arr[ai_arr[i]] = I_STATE;        // 改变种子节点的状态 ，已激活
        aactive_size++;
    }
    //权重级联模型
    at_size = 0;                      //t+1活动节点为0
        ...
        //即完成当前活动节点在下一步对邻居的激活
    {
        ...
        for(int j = 0; j < ak_out; j++)              //对第i个活动节点的邻居进行循环
        {
            ...
            if (astate_arr[neigh] == S_STATE)                    //如果此邻居是未激活的
            {
                ...
                {
                    astate_arr[neigh] = SI_STATE;               //改变活动节点的邻居neigh的状态为下一步激活，实质的扩散
    ...
    {    //i即当前轮节点激活过程结束，变成过去激活节点
        astate_arr[ai_arr[i]] = R_STATE;          //设置节点状态为激活过
        ar_arr[ar_size++] = ai_arr[i];               // 加入到先前激活的数组中
    }
    ai_size = 0;                    //t时刻活动数量为0
    for(int i = 0; i < at_size; i++)     //对t+1时刻活动节点的遍历
    {  //i+1即刚激活的节点准备去激活其他节点
        astate_arr[at_arr[i]] = I_STATE;   //设置状态为激活
...
//    randomOrder(brand_arr, bi_size);       //rand_arr为k个种子数组 ，i_size是当前活动节点数量
        ...
        //即完成当前活动节点在下一步对邻居的激活
    {
        ...
        for(int j = 0; j < bk_out; j++)              //对第i个活动节点的邻居进行循环
        {
            ...
            if (bstate_arr[neigh] == S_STATE)                    //如果此邻居是未激活的
            {
                int k_in = g->node_array[neigh].k_in;           //则把此邻居的入度赋值给k_in
                //double pp = 1.0 / k_in;  //激活概率为入度的倒数  重力级联模型
                double pp = p;      //独立级联模型   pp是成功概率
                ...
                if(rand_float > pp)                           //激活概率小于成功概率，则表示激活
                {
                    bstate_arr[neigh] = SI_STATE;               //改变活动节点的邻居neigh的状态为下一步激活，实质的扩散
    ...
    {    //i即当前轮节点激活过程结束，变成过去激活节点
        bstate_arr[bi_arr[i]] = R_STATE;          //设置节点状态为激活过
        br_arr[br_size++] = bi_arr[i];               // 加入到先前激活的数组中
    }
    bi_size = 0;                    //t时刻活动数量为0
    for(int i = 0; i < bt_size; i++)     //对t+1时刻活动节点的遍历
    {  //i+1即刚激活的节点准备去激活其他节点
        bstate_arr[bt_arr[i]] = I_STATE;   //设置状态为激活
