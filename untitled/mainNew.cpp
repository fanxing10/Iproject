...
//	time_t time_start = time(NULL);
    ...
    for(int k=0;k<s;k++)  //对步数的循环
    {
        ...
        for(int i=0;i<num;i++)
        {
            ...
            {
                if(state_arr[state_pair.key]!=S_STATE)
                {
                    aseed_arr[i]=state_pair.key;
                    float m_gain=mc_influence(i+1)-total_inf;
...
float mc_influence(int k)
{
    ...
    int *sstate_arr = new int[g->num_nodes]; // the state of nodes
    memset(sstate_arr, S_STATE, g->num_nodes * sizeof(int)); // initialize the state array
    ...
    for(int r = 0; r < NUM_SIMUS; r++)
    {
        ...
        {
            sstate_arr[r_arr[i]] = S_STATE;
        }
        r_size = 0;
        // initialize the seed nodes
        for(int i = 0; i < k; i++)
        {
            i_arr[i_size++] = aseed_arr[i];
            sstate_arr[i_arr[i]] = I_STATE;
        }
        while(i_size > 0)
        {   //权重级联模型
            active_size += i_size;
            ...
            for(int i = 0; i < i_size; i++)
            {    //i即当前轮节点激活过程结束，变成过去激活节点
                sstate_arr[i_arr[i]] = R_STATE;
                r_arr[r_size++] = i_arr[i];
            }
            i_size = 0;
            for(int i = 0; i < si_size; i++)
            {  //i+1即刚激活的节点准备去激活其他节点
                sstate_arr[si_arr[i]] = I_STATE;
